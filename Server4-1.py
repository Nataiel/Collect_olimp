...
def setup_flask_app():
    ...
    # Изменён CLASS_PAGE_TEMPLATE - добавлен JavaScript для массового проставления галочек
    CLASS_PAGE_TEMPLATE = """
    <!DOCTYPE html>
    <html>
    <head>
        ...
        <style>
            ...
            /* === НОВЫЙ СТИЛЬ: курсор-указатель для заголовков предметов === */
            .subject-header {
                ...
                min-width: 1px;
                min-height: 140px;
                cursor: pointer;  /* Добавлено: указатель при наведении */
                user-select: none; /* Добавлено: запрет выделения текста */
            }
            /* === НОВЫЙ СТИЛЬ: подсветка заголовка при наведении === */
            .subject-header:hover {
                background: #e8f4fd;  /* Добавлено: подсветка при наведении */
            }
            .student-name {
                width: 15%;
                background: #f8f9fa;
    ...
    <body>
        <div class="header">
            ...
            <div style="font-size: 14px; color: #7f8c8d; margin-top: 5px;">
                💡 Кликните по названию предмета, чтобы отметить/снять всех учеников
            </div>
        </div>
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <!-- === ИЗМЕНЕНИЕ: Добавлен onclick и атрибут data-subject === -->
                            <th class="subject-header" 
                                onclick="toggleColumn('{{ subject|escape }}')"
                                data-subject="{{ subject|escape }}"
                                title="Кликните для массового выбора предмета {{ subject }}">
                                {{ subject }}
                            </th>
                            {% endfor %}
                        </tr>
        ...
        <script>
            ...
            // ===== НОВАЯ ФУНКЦИЯ: массовое переключение галочек в столбце =====
            function toggleColumn(subjectName) {
                // Находим все чекбоксы, у которых name заканчивается на "_subjectName"
                // (в name используется шаблон "student_subject")
                const checkboxes = document.querySelectorAll('input[type="checkbox"]');
                const targetCheckboxes = [];

                // Собираем все чекбоксы для данного предмета
                checkboxes.forEach(cb => {
                    // Имя чекбокса имеет формат "ФИО_Предмет"
                    // Проверяем, что имя заканчивается на "_" + subjectName
                    if (cb.name.endsWith('_' + subjectName)) {
                        targetCheckboxes.push(cb);
                    }
                });

                if (targetCheckboxes.length === 0) {
                    console.warn('Не найдены чекбоксы для предмета:', subjectName);
                    return;
                }

                // Определяем текущее состояние: все ли чекбоксы отмечены?
                const allChecked = targetCheckboxes.every(cb => cb.checked);
                // Если все отмечены - снимаем все, иначе - отмечаем все
                const newState = !allChecked;

                // Применяем новое состояние ко всем чекбоксам в столбце
                targetCheckboxes.forEach(cb => {
                    cb.checked = newState;
                });

                // Небольшая визуальная обратная связь
                console.log(`Предмет "${subjectName}": установлено ${newState ? '✅' : '❌'} для ${targetCheckboxes.length} учеников`);
            }
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // ===== НОВЫЙ КОД: Добавляем подсказку о возможности массового выбора =====
                // Находим все заголовки предметов и добавляем им атрибут title,
                // если он ещё не установлен (дублируем, но на всякий случай)
                document.querySelectorAll('.subject-header').forEach(th => {
                    if (!th.hasAttribute('title')) {
                        const subject = th.textContent.trim();
                        th.setAttribute('title', `Кликните для массового выбора предмета "${subject}"`);
                    }
                });
            });
        </script>
...
- Нажмите "Выбрать Excel файл" — загрузите список учеников (листы с названиями классов: 4А, 5Б, 8В...). 
  *Используется выгрузка из ГИС СО "ЕЦП"  Электронный журнал. Файл формируется в разделе: Учет учащихся и педагогов → Классы → Сформировать списки: Для всех классов, Поле в отчёте - любое (обычно выбирают Пол)
